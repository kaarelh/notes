...
1. Let’s think of all mathematical objects as collections.^[I'll speak of "collections" instead of "sets" to make it clear that we will not necessarily be assuming something like the ZFC conception of a set from the get-go.] This is a standard choice in mathematics when one wants to be precise/concrete — there are standard ways to think of numbers, groups, graphs, functions, etc as sets. It seems quite fair to think of this as a convention (as opposed to being a fundamental claim about something).^[Note that at this point, we've taken the view that all mathematical objects are collections, but we haven't yet taken a position on whether all collections are mathematical objects.]
